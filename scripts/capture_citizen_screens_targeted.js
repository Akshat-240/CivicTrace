...
async function main() {
  ...
  console.log('Spawning Chrome for targeted captures...');
  ...
  try {
    ...
    // Capture Screen 02 (Report): Pan left once from center
    await pan(500, 2000);
    let shot = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('citizen_targeted_screen_2.png', Buffer.from(shot.result.data, 'base64'));
    console.log('Saved citizen_targeted_screen_2.png');

    // Reset by panning right twice to get to Screen 05 (Feedback)
    await pan(2200, 500);
    await pan(2200, 500);
    shot = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('citizen_targeted_screen_5.png', Buffer.from(shot.result.data, 'base64'));
    console.log('Saved citizen_targeted_screen_5.png');

    // Pan right once more to get Screen 06 (Settings) in center
    await pan(2200, 700);
    shot = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('citizen_targeted_screen_6.png', Buffer.from(shot.result.data, 'base64'));
    console.log('Saved citizen_targeted_screen_6.png');
