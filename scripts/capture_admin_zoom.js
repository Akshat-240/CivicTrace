...
async function main() {
  ...
  console.log('Spawning Chrome for Admin zoom capture...');
  ...
  try {
    ...
    // Zoom to 100%: Ctrl + 0 (modifiers: 2)
    console.log('Zooming to 100%...');
    ...
    fs.writeFileSync('admin_zoom_100.png', Buffer.from(shot.result.data, 'base64'));
    console.log('Saved admin_zoom_100.png');
    ...
    // Capture multiple horizontal panning segments at 100%
    // Let's first pan to far left
    for (let i = 0; i < 5; i++) {
      await pan(600, 2400);
    }
    shot = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('admin_seg_0.png', Buffer.from(shot.result.data, 'base64'));
    console.log('Saved admin_seg_0.png');

    // Now pan right sequentially and capture
    for (let s = 1; s <= 8; s++) {
      await pan(2200, 600);
      shot = await send('Page.captureScreenshot', { format: 'png' });
