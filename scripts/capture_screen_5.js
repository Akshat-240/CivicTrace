...
async function main() {
  ...
  try {
    ...
    // Pan right twice to hit Screen 5 (Incident Verification)
    for (let i = 0; i < 2; i++) {
      ...
      await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 700, y: 700, button: 'left' });
      await send('Input.dispatchKeyEvent', { type: 'keyUp', code: 'Space', key: ' ', windowsVirtualKeyCode: 32 });
      await new Promise(r => setTimeout(r, 1000));
    }

    const screen5Shot = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('screen_5.png', Buffer.from(screen5Shot.result.data, 'base64'));
    console.log('Saved screen_5.png');

    // Also let's scroll screen 6 down to see the bottom of settings
    // Pan right one more time for screen 6
    await send('Input.dispatchKeyEvent', { type: 'rawKeyDown', code: 'Space', key: ' ', windowsVirtualKeyCode: 32 });
    await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: 2000, y: 700, button: 'left', clickCount: 1 });
    await send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: 700, y: 700 });
    await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 700, y: 700, button: 'left' });
    await send('Input.dispatchKeyEvent', { type: 'keyUp', code: 'Space', key: ' ', windowsVirtualKeyCode: 32 });
    await new Promise(r => setTimeout(r, 1000));

    // And pan up to see bottom of screen 6
    await send('Input.dispatchKeyEvent', { type: 'rawKeyDown', code: 'Space', key: ' ', windowsVirtualKeyCode: 32 });
    await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: 1200, y: 1200, button: 'left', clickCount: 1 });
    await send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: 1200, y: 500 });
    await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 1200, y: 500, button: 'left' });
    await send('Input.dispatchKeyEvent', { type: 'keyUp', code: 'Space', key: ' ', windowsVirtualKeyCode: 32 });
    await new Promise(r => setTimeout(r, 1000));

    const screen6Bottom = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('screen_6_bottom.png', Buffer.from(screen6Bottom.result.data, 'base64'));
    console.log('Saved screen_6_bottom.png');
