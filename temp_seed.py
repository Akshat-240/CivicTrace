...
async def seed_incidents(
    ...
    for row in rows:
        ...
        # CivicTrace domain invariant:
        #   IF jurisdiction_id IS NULL THEN authority_id MUST be NULL.
        #   Authority must always derive from a resolved jurisdiction, never
        #   be assigned independently from a raw CSV field.
        ward_src = row.get("ward_id", "")
        zone_src = row.get("zone_id", "")
        jurisdiction_id = jur_id_map.get(ward_src) or jur_id_map.get(zone_src)

        if jurisdiction_id is not None:
            # Jurisdiction resolved → honour the CSV authority field.
            auth_src = row.get("authority_id", "")
            authority_id = auth_id_map.get(auth_src)
        else:
            # No jurisdiction resolved → must NOT assign an authority.
            # Assigning authority_id without jurisdiction_id would violate
            # ck_incidents_jurisdiction_authority_invariant and misrepresent
            # how the incident was routed.
            authority_id = None
            auth_src_raw = row.get("authority_id", "")
            if auth_src_raw:
                print(
                    f"    [warn ] {src_id}: authority '{auth_src_raw}' ignored "
                    f"(no jurisdiction resolved from ward='{ward_src}' zone='{zone_src}')"
                )
