...
async function main() {
  ...
  try {
    ...
    // Pan way to the left (3 times) to see Screen 1 fully with its sidebar and left margin
    for (let i = 0; i < 3; i++) {
    ...
    const screen1Shot = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('screen_1.png', Buffer.from(screen1Shot.result.data, 'base64'));
    console.log('Saved screen_1.png');
